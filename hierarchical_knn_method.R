...
for(k in 1:10){
  ...
  index = 1
  for(i in seq(1,dim(sampling)[1])){
    j = i+1
    while(j<=dim(sampling)[1]){
      a = as.vector(unlist(sampling[i,]))
      b = as.vector(unlist(sampling[j,]))
  ...
  filename1 = paste("data\\DTW\\hierarchical_clustering_result_DTW_sample_",k*100,".txt",sep="")
  filename2 = paste("data\\DTW\\hierarchical_clustering_labels_2_8_DTW_sample_",k*100,".txt",sep="")
